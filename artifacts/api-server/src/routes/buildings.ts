...
router.get("/buildings/:id", (req, res): void => {
  ...
  const building = buildings.find((item) => item.id === params.data.id);
  if (!building) {
...
router.get("/dashboard/summary", (_req, res): void => {
  const result = {
    ...
    verified: buildings.filter((building) => building.audit.status === "verified").length,
    openGaps: buildings.reduce((count, building) => count + building.report.gaps.length, 0),
    averageRating: Number((buildings.reduce((sum, building) => sum + building.rating, 0) / buildings.length).toFixed(1)),
